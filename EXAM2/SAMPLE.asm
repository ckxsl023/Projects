...
.stack
.data
s1   db  'Enter 1st String: $'
s11   db  'Enter 2nd String: $'
s2   db  0ah,0dh,'$'
buff db	 20,?,20 dup(?) 
temp db	 20 dup(?)
...
m 	proc
	...
	mov ah,9	; display 'Enter 1st String'
	mov dx,offset s1
	int 21h
...
ulit:	movsb		;copy the character pointed to by si to the location pointed to by di in each iteration
	...
	mov al,' '	;append the $ at the end of the string in 'temp'	ADD S
	stosb
	...
	mov ah,9	; display 'Enter 2nd String'
	mov dx,offset s11
	int 21h
...
mov ah,0ah	; get string input
	...
	mov ch,0
	mov cl,al	;copy al to cl; the number in cl will now serve as the counter for the loop
ulit2:	movsb		;copy the character pointed to by si to the location pointed to by di in each iteration
	loop ulit2
